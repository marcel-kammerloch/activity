...
def getFileSha(debug=False):
    ...
    return  str(response.json()["sha"]) if status == 200 else (status if debug is True else None)
...
def main():
    try:
        newContent = incrementNumberOfFile()
        fileSha = getFileSha()
...
def __test__():
    try:
        ...
        fileSha = getFileSha(True)
        if(int.is_integer(fileSha)): raise Exception(f"fileSha is not a string. Status: {fileSha}")
    except Exception as e:
        print(f"Error: {e}")
